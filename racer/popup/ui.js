...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const scanStrategySelect = document.getElementById('scan-strategy');
    const strategyInfoDiv = document.getElementById('strategy-info');
    ...
    scanStrategySelect.addEventListener('change', () => {
        const val = scanStrategySelect.value;
        if (val === 'standard') {
            strategyInfoDiv.textContent = "Standard concurrent fetch requests.";
        } else if (val === 'last-byte') {
            strategyInfoDiv.textContent = "Low-level packet syncing. Requires H2/H3/Modern Browser. Forces POST method.";
        }
    });
    ...
    startScanBtn.addEventListener('click', () => {
        ...
        const strategy = scanStrategySelect.value;
    ...
    function renderTraffic(requests) {
        ...
        requests.slice().reverse().forEach((req, index) => {
            ...
            if (req.statusCode) {
                ...
                statusSpan.className = 'traffic-status';
                statusSpan.textContent = req.statusCode;
                if (req.statusCode >= 400) statusSpan.style.color = '#dc3545';
                else if (req.statusCode >= 200 && req.statusCode < 300) statusSpan.style.color = '#28a745';
            ...
            // URL span is already appended but we want status code before actions?
            // Let's reorder: Method | URL | Status | Actions
            div.appendChild(urlSpan);

            // Note: Status span was appended to div in the previous block if present.
            // But we want it visually after URL, before Actions.
            // Since we appended it directly to div above, it's currently after URL (if we follow order).
            // But we haven't appended actions yet.
